...
const GetCancelationData = () => {
  const [referenceId, setReferenceId] = useState(""); // Reference ID as input field state
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('/admin/cancellation/data', {
        referenceId: referenceId // Send the input field value to backend
      });
      ...
      if (response.data.status) {
        ...
        setReferenceId(""); // Clear the input field after success
      } else {
        setError(response.data.message || "Failed to retrieve cancellation data");
      }
    } catch (err) {
      console.error('Error fetching cancellation data:', err);
      const errorMessage = err.response?.data?.message || 
                         err.response?.data?.errors || 
                         "Failed to retrieve cancellation data";
      ...
      <div className="max-w-full p-6">
        <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
          <div className="bg-gradient-to-tr from-gray-400 to-black py-4 px-6">
            <h2 className="text-3xl font-semibold text-white">Get Cancellation Data</h2>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="mb-6">
              ...
              <input
                ...
                value={referenceId} // Controlled input field
                onChange={(e) => setReferenceId(e.target.value)} // Update state on change
...
export default GetCancelationData;